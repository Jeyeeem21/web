...
try {
    ...
    ob_clean();
    echo '<div class="bg-red-50 border border-red-200 text-red-800 px-3 py-2 rounded-md text-sm m-4">';
    ...
    if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] == 'add') {
            ...
            try {
                ...
                $sql = "INSERT INTO patient (name, email, phone, address, birthdate, age, photo, gender, status, created_at) 
...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("UPDATE patient SET status = 0 WHERE id = :id");
        $stmt->execute([':id' => $id]);
        $pdo->commit();
...
try {
    $stmt = $pdo->query("SELECT * FROM patient WHERE status = 1 ORDER BY created_at DESC");
...
<div id="records" class="space-y-6">
    <h2 class="text-xl font-medium text-gray-800">Patient Records</h2>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div class="overflow-x-auto">
            <table id="patientsTable" class="min-w-full divide-y divide-gray-200 mobile-card-view">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Last Visit</th>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                    <?php foreach ($patients as $patient): ?>
                    <tr class="hover:bg-gray-50" id="patient-row-<?php echo $patient['id']; ?>">
                        <td class="px-4 py-2" data-label="Photo">
    ...
    <div class="bg-white rounded-lg shadow p-6 mt-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-medium text-gray-900">Patient User Accounts</h3>
        ...
        <div class="overflow-x-auto">
            <table id="patientUsersTable" class="min-w-full divide-y divide-gray-200 mobile-card-view">
                <thead>
                    <tr>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    $stmt = $pdo->query("
                        ...
                        FROM users u 
                        JOIN patient p ON u.patient_id = p.id 
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-2" data-label="Username">
                            <div class="text-sm text-gray-900"><?php echo htmlspecialchars($user['username']); ?></div>
                        </td>
                        <td class="px-4 py-2" data-label="Patient Name">
                            <div class="text-sm text-gray-900"><?php echo htmlspecialchars($user['patient_name']); ?></div>
                        </td>
                        ...
                        <td class="px-4 py-2" data-label="Actions">
                            <div class="flex space-x-2">
                                <button type="button" class="text-blue-600 hover:text-blue-800" onclick="openEditUserModal(<?php echo $user['id']; ?>, '<?php echo htmlspecialchars($user['username']); ?>', <?php echo $user['patient_id']; ?>)">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
    ...
    <div id="patientModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
            <div class="mt-3">
                ...
                <form id="patientForm" method="POST" enctype="multipart/form-data" class="mt-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="email" name="email" id="email" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Phone</label>
                        <input type="text" name="phone" id="phone" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Address</label>
                        <textarea name="address" id="address" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"></textarea>
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Birthdate</label>
                        <input type="date" name="birthdate" id="birthdate" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Gender</label>
                        <select name="gender" id="gender" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500" onchange="toggleOtherGender()">
                    ...
                    <div id="otherGenderDiv" class="mb-4 hidden">
                        <label class="block text-sm font-medium text-gray-700">Specify Gender</label>
                        <input type="text" name="other_gender" id="other_gender" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                    </div>
            ...
            <div class="mt-3">
                ...
                <form id="addUserForm" method="POST" class="mt-4">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="patient_id" id="addUserPatientId" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
        ...
        <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
            <div class="mt-3">
                ...
                <form id="editUserForm" method="POST" class="mt-4">
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Patient</label>
                        <select name="patient_id" id="editUserPatientId" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                            <option value="">Select Patient</option>
                            <?php
                            // Get all active patients
                            $stmt = $pdo->query("SELECT * FROM patient WHERE status = 1 ORDER BY name");
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Username</label>
                        <input type="text" name="username" id="editUserUsername" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                    </div>
                    
                    <div class="mb-4 relative">
                        <label class="block text-sm font-medium text-gray-700">New Password (leave blank to keep current)</label>
                        <input type="password" name="password" id="editUserPassword" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 pr-10">
                        <button type="button" id="toggleEditUserPassword" class="absolute inset-y-0 right-0 flex items-center pr-3 mt-6 text-gray-500 hover:text-gray-700">
                    ...
                    <div class="flex justify-end space-x-2">
                        <button type="button" onclick="closeEditUserModal()" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Cancel</button>
...
@media (max-width: 640px) {
    ...
    /* Status badge mobile adjustments */
    .mobile-card-view .px-2.inline-flex {
        margin-left: auto;
    }
}
